...
class VectorDBTool(BaseTool):
    ...
    def initialize(self):
        ...
        # 定义函数工具
        @function_tool
        def vector_store_add(texts: Annotated[List[str], "要存储的文本列表"],
                            metadatas: Annotated[str, "可选的元数据，JSON字符串格式"] = "[]") -> str:
            """将文本添加到向量数据库"""
            try:
                import json
        ...
        @function_tool
        def vector_store_search(query: Annotated[str, "搜索查询"],
                               n_results: Annotated[int, "返回结果数量，默认使用配置中的值"] = None,
                               full_content: Annotated[bool, "是否显示完整内容，默认为False（截断显示）"] = False) -> str:
            """在向量数据库中搜索相似文本"""
            try:
                ...
                for i, (doc, metadata, distance) in enumerate(zip(
                    ...
                    if full_content:
                        ...
                        response += f"   内容: {doc[:200]}{'...' if len(doc) > 200 else ''}\n"
                    if metadata:
        ...
        def vector_store_get(doc_id: Annotated[str, "文档ID"]) -> str:
            ...
            try:
                ...
                for id_, doc, metadata in zip(data['ids'], data['documents'], data['metadatas']):
                    if id_ == doc_id:
        ...
        @function_tool
        def vector_store_list(full_content: Annotated[bool, "是否显示完整内容，默认为False（截断显示）"] = False) -> str:
            """列出向量数据库中的所有文档"""
            try:
                data = self.get_all()
                ...
                for i, (doc_id, doc, metadata) in enumerate(zip(
                    ...
                    if full_content:
                        ...
                        response += f"   内容: {doc[:100]}{'...' if len(doc) > 100 else ''}\n"
                    if metadata:
                        response += f"   元数据: {metadata}\n"
                    response += "\n"
